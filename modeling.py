...
class BertForTABSAJoint(nn.Module):
	...
	def forward(self, input_ids, input_Proids, token_type_ids, Protoken_type_ids, attention_mask, attention_Promask, labels, ner_labels, ner_mask, eval_flag):
		all_encoder_layers, pooled_output = self.bert(input_ids, token_type_ids, attention_mask)# 编码器输出的表示和池化层输出的表示
		all_encoder_layers_2, pooled_output_2 = self.bert(input_Proids, Protoken_type_ids, attention_Promask)#-----第二次
		# get the last hidden layer 获取序列中每个标记的表示
		sequence_output = all_encoder_layers[-1]
		sequence_output_2 = all_encoder_layers_2[-1]
		# cross a dropout layer 随机失活
		sequence_output = self.dropout(sequence_output)
		sequence_output_2 = self.dropout(sequence_output_2)
        
		#seq_length = sequence_output.size(1)
		#pooled_output = torch.avg_pool1d(sequence_output.transpose(1, 2), kernel_size=seq_length).squeeze(-1)
		#pooled_output_2 = torch.avg_pool1d(sequence_output_2.transpose(1, 2), kernel_size=seq_length).squeeze(-1)
		...
		logits_2 = self.classifier(pooled_output_2)# 第二次输出

		ner_logits = self.ner_hidden2tag(sequence_output)# 编码器输出Tn,通过Linear得到Pn
		ner_logits_2 = self.ner_hidden2tag(sequence_output_2)
		ner_logits.reshape([-1, self.max_seq_length, self.num_ner_labels])
		ner_logits_2.reshape([-1, self.max_seq_length, self.num_ner_labels])

		loss_fct = CrossEntropyLoss()
		# cross entropy loss for classifier
		#loss = loss_fct(logits, labels)
		ce_loss = 0.5 * (loss_fct(logits, labels) + loss_fct(logits_2, labels))

		kl_loss = BertForTABSAJoint.compute_kl_loss(logits, logits_2)
		...
		ner_ce_loss = 0.5 * (ner_loss_fct(ner_logits.view(-1, self.num_ner_labels), ner_labels.view(-1)) + ner_loss_fct(ner_logits_2.view(-1, self.num_ner_labels), ner_labels.view(-1)))
		ner_kl_loss = BertForTABSAJoint.compute_kl_loss(ner_logits, ner_logits_2)
		ner_loss = ner_ce_loss + 2 * ner_kl_loss

		return loss, ner_loss, logits, ner_logits

# BERT + AT
class BertForTABSAJoint_AT(nn.Module):
	def __init__(self, config, num_labels, num_ner_labels):
		super(BertForTABSAJoint_AT, self).__init__()
		self.bert = BertModel(config)
		self.dropout = nn.Dropout(config.hidden_dropout_prob)
		self.classifier = nn.Linear(config.hidden_size, num_labels) # num_labels is the type sum of 0 & 1
		self.ner_hidden2tag = nn.Linear(config.hidden_size, num_ner_labels) # num_ner_labels is the type sum of ner labels: TO or BIO etc
...
class BertForTABSAJoint_CRF(nn.Module):
	...
	def forward(self, input_ids, token_type_ids, attention_mask, labels, ner_labels, ner_mask, eval_flag):
		all_encoder_layers_1, pooled_output_1 = self.bert(input_ids, token_type_ids, attention_mask)
		all_encoder_layers_2, pooled_output_2 = self.bert(input_ids, token_type_ids, attention_mask)#-----第二次
		# get the last hidden layer
		sequence_output_1 = all_encoder_layers_1[-1]
		sequence_output_2 = all_encoder_layers_2[-1]
		# cross a dropout layer
		...
		ner_logits_1 = self.ner_hidden2tag(sequence_output_1)
		ner_logits_2 = self.ner_hidden2tag(sequence_output_2)

		# the CRF layer of NER labels
		ner_loss_list_1 = self.CRF_model(ner_logits_1, ner_labels, ner_mask.type(torch.ByteTensor).cuda(), reduction='none')
		ner_loss_list_2 = self.CRF_model(ner_logits_2, ner_labels, ner_mask.type(torch.ByteTensor).cuda(), reduction='none')
		ner_loss_1 = torch.mean(-ner_loss_list_1)
		ner_loss_2 = torch.mean(-ner_loss_list_2)
		ner_predict_1 = self.CRF_model.decode(ner_logits_1, ner_mask.type(torch.ByteTensor).cuda())
		ner_predict_2 = self.CRF_model.decode(ner_logits_2, ner_mask.type(torch.ByteTensor).cuda())
		...
		kl_loss = BertForTABSAJoint_BiLSTM_CRF.compute_kl_loss(logits_1, logits_2)
...
class BertForTABSAJoint_BiLSTM_CRF(nn.Module):
	def __init__(self, config, num_labels, num_ner_labels):
		...
		# CRF
		self.CRF_model = CRF(num_ner_labels, batch_first=True)
		#初始dim=128，layer=1    最好结果为7层lstm，hidden_size=128，200效果不佳，原因可能是超过了潜在特征数量
	...
	#LDA主题模型-------------------------------------------------
	def lda_train(corpus, num_topics, workers, id2word):
		lda = LdaMulticore(corpus=corpus, num_topics=num_topics, workers=workers, id2word=id2word)
