...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, text_Prob=None, label=None, ner_labels_a=None):
...
class Semeval_Processor(DataProcessor):# 数据预处理
    ...
    def _create_QAexamples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            #----
            Proquestion = "does the polarity of the aspect {aspect1} {aspect2} belongs to {polarity}".format(aspect1=temp[0],aspect2=temp[1],polarity=temp[-1])

            #text_b = tokenization.convert_to_unicode(str(line_arr[2])) # category_polarity
            text_b = tokenization.convert_to_unicode(str(question))
            #----
            text_Prob = tokenization.convert_to_unicode(str(Proquestion))

            label = tokenization.convert_to_unicode(str(line_arr[1])) # yes or no  0/1
            ner_labels_a = tokenization.convert_to_unicode(str(line_arr[4])) # ner tags
            # 获取每一条数据对象接在examples后面
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, text_Prob=text_Prob, label=label, ner_labels_a=ner_labels_a))
        return examples
