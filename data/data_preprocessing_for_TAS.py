...
def create_dataset_file(input_path, output_path, input_file, output_file, compose_set):
	one_data_nums = 0 # 1的数量
	zero_data_nums = 0# 0的数量
	max_len = 0
	entity_sum = 0 # 实体数量
	...
	with open(os.path.join(input_path, TXT_file(input_file)), 'r', encoding='utf-8') as fin, open(os.path.join(output_path, TSV_file(output_file)), 'w', encoding='utf-8') as fout:
		fout.write('\t'.join(['sentence_id', 'yes_no', 'aspect_sentiment', 'sentence', 'ner_tags', 'target']))# 这里需要添加target字段
		fout.write('\n')
		fin.readline()
		pre_start = False
		pre_sentence_id = 'XXX'	# 前一行句子的id
		pre_sentence = 'XXX'
		target = 'XXX' # 新增------------------------------------------------
		record_of_one_sentence = set()	# the set of aspect&sentiment that this sentence contains 包含的一系列方面和情感
		record_of_one_sentence_ner_tag = {}	# the NER tags of the set of aspect&sentiment that this sentence contains
		for line in fin:
			line_arr = line.strip().split('\t')
			sentence_id = line_arr[0]# 获取id line_arr[0]
			if sentence_id != pre_sentence_id:	# this is a new sentence
				if pre_start == True:
					for x in compose_set:
						# create yes line of this sentence 应该在这里最后添加target
						if x in record_of_one_sentence:
							fout.write(pre_sentence_id + '\t' + '1' + '\t' + x + '\t' + pre_sentence + '\t' + record_of_one_sentence_ner_tag[x] + '\t' + target + '\n')
							one_data_nums += 1
						# create no line
						else:
							fout.write(pre_sentence_id + '\t' + '0' + '\t' + x + '\t' + pre_sentence + '\t' + ' '.join(['O']*len(pre_sentence.split())) + '\t' + target + '\n')
							zero_data_nums += 1
			...
			if line_arr[6] == 'yes':	# entailed == yes
				...
				ner_tags = ['O'] * len(sentence)# O数量
				target = gold_target# 新增---------------------------------------------------------
